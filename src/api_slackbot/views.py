...
from api_slackbot.services.exception.exception_engine_builder import (
    InteractingExceptionEngine,
    MentionExceptionEngine,
)
...
from api_slackbot.services.slack.user_request.handler import (
    UserLeaveRequestHandler,
    UserWFHRequestHandler,
)
...
def get_profile_by_slack_id_and_update_if_missing(user_slack_id):
    ...
    # Get user profile from HRM by user email, using superadmin for quicker testing
...
def create_temp_leave_request_and_send_response(
    ...
    if existed_request is not None:
        ...
        if response_to_duplicated_request.get("is_exception"):
            ...
            ).return_http_no_retires_response("Exception raised")
        else:
            ...
            return Response(
                dict(msg="Sent successfully to requesting user"),
    ...
    # Because WFH does not counted as a leave type, so we save the field
    ...
    if response_to_valid_request.get("is_exception"):
        ...
        slack_service.send_ephemeral_message(
            user=slack_user_id,
            text="Here is your request information!",
...
def handling_action_event(request):
    ...
    try:
        ...
        if user_action.get("action_type") == SlackActionId.BUTTON_APPROVE_REQUEST:
            ...
            ).action_handling(line_manager_action_type=Workday.STATUS_APPROVED)
        ...
        if user_action.get("action_type") == SlackActionId.BUTTON_DENY_REQUEST:
            return LineManagerActionService(
                ...
                profile=profile,
                list_dates=json.loads(action_data.get("value")).get("list_dates"),
            ).action_handling(line_manager_action_type=Workday.STATUS_REJECTED)
    except Exception as e:
        return ExceptionHandler(
