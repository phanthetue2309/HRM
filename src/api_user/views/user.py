...
class UserViewSet(BaseViewSet):
    ...
    Can define scope if need, check in scopes.json file if scope doesnt exist please add it follow template
    """
    ...
    required_alternate_scopes = {
        ...
        "search": [["user:view_public_user_information_list"]],
        "get_non_paginate": [["user:view_public_user_information_list"]],
        "get_latest_five": [["user:view_public_user_information_list"]],
        "search_non_paginate": [["user:view_public_user_information_list"]],
        "activate": [["user:activate_user"]],
        "change_password": [["user:access_personal"]],
        "get_project_managers": [["user:view_public_user_information_list"]],
        "get_all_title": [["user:view_public_user_information_list"]],
    ...
    def get_project_managers(self, request, *args, **kwargs):
        queryset = self.queryset.filter(title=TitleConstant.TITLES[2][0])
        data = self.get_serializer(queryset, many=True).data
