...
class GetUserService extends BaseService {
  ...
  get(page_size, page, active = 1) {
    const res = this.request().get(`${this.entity}/?page_size=${page_size}&page=${page}&active=${active}`)
    return res
  }
  ...
  getLineManager(id) {
    return this.request({auth: true}).get(`${this.entity}/get_line_manager?id=${id}`);
  }

  getUserIncludeLineManagerName(userId) {
    return this.request({auth: true}).get(`${this.entity}/get_users_include_line_manager?user_id=${userId}`);
  }
}
