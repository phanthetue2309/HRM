<template>
  <div>
    ...
    <div class="d-flex justify-content-end mt-5">
      <button type="button" class="btn btn-primary btnHandleData" @click="addDataToTable">
    ...
    <table class="tableData">
      ...
      <transition-group tag="tbody" name="list" class="text-center" mode="out-in">
        <tr v-for="(level, index) in levels" :key="index">
          <td>{{ level.id }}</td>
          <td>
            <span v-if="level.editMode">
              <input type="text" placeholder="Placeholder" v-model="level.name" class="m-auto"/>
            </span>
            <span v-else>{{ level.name }} </span>
          </td>
          <td>
            <span v-if="level.editMode">
              <input type="number" placeholder="Placeholder" v-model="level.weight" class="m-auto"/>
            </span>
          ...
          <td>
           ...
           <template v-slot:default>
            <div v-if="!level.editMode">
              <img :src="require('@/static/images/IconEdit.svg')" class="mr-3" @click="editData(level)">
              <img :src="require('@/static/images/IconDelete.svg')" class="ml-3" @click="deleteData(level.id)">
            </div>
            <div v-else>
