<template>
  <div>
    <restricted-view :scopes="['skill_definition:edit']">
      <template v-slot:default>
        <div class="d-flex justify-content-end mt-5">
          <button type="button" class="btn btn-primary" @click="dialogAdd = true">
    ...
    <table class="tableData mt-2">
      ...
      <transition-group name="list" mode="out-in" tag="tbody" class="text-center">
        <template v-for="skillDefinition in skillDefinitions">
          <tr v-for="(item,index) in skillDefinition.data" :key="item.id">
            <td :rowspan="skillDefinition.data.length" v-if="index===0">{{ skillDefinition.name }}</td>
            <td>{{ item.level.name }}</td>
            <td class="text-left p-2" style="white-space: pre-wrap">{{ item.requirements }}</td>
            <td :rowspan="skillDefinition.data.length" v-if="index===0">
              <restricted-view :scopes="['skill_definition:edit']">
                <template v-slot:default>
                  <img :src="require('@/static/images/IconEdit.svg')" @click="editData(skillDefinition)">
                  <img :src="require('@/static/images/IconDelete.svg')" @click="dialogDelete = true">
                </template>
              </restricted-view>
            </td>
    ...
    <el-dialog
        ...
        :before-close="handleClose"
    >
      <EditSkillDefinition :skillDefinitionEdit="skillDefinitionEdit" @update-skill="updateData"/>
    </el-dialog>
    ...
    async deleteData(skill_id) {
      try {
        ...
        if (res.status === 204) {
          ...
          const index = this.skillDefinitions.findIndex(skill => skill.id === skill_id)
          if (index >= 0) {
