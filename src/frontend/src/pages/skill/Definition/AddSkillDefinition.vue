  ...
  <form @submit.prevent="addData(currentDefinition)">
    ...
    <div class="col p-4 content">
      <div class="p-2">
        ...
        <el-input placeholder="Please fill input" v-model="currentDefinition.skill.name" class="inputName"></el-input>
      </div>
      <div class="p-2 border-definition">
        ...
        <div class="mt-3">
          <span class="input-label" style="line-height: 54px">Requirements:</span>
          <el-input
            type="textarea"
        ...
        <div class="d-flex justify-content-center align-items-center btnAddDataTable"
        ...
        <table>
          ...
          <tr class="item-row" v-for="(item, index) in arrDefinition" :key="index">
            <td>
            ...
            <td>
              <div v-if="!item.editMode">
                <img :src="require('@/static/images/IconEdit.svg')" @click="editDataTable(item)">
                <img :src="require('@/static/images/IconDelete.svg')" @click="deleteDataTable(item)">
              </div>
              <div v-else>
      ...
      <button type="submit" class="d-flex justify-content-center align-items-center addData">
        Save
      </button>
    </div>
  </form>
  ...
  methods: {
    updateRemainingLevels() {
      ...
      this.arrDefinition.forEach((definition) => {
        let index = this.remainingLevels.findIndex((level) => level.id === definition.level.id)
    ...
    editDataTable(levelRow) {
      ...
      this.editLevels = this.levels.filter(level => this.arrDefinition.filter(e => e.level.name === level.name).length === 0)
      this.editLevels.push(Object.assign({}, levelRow.level))
    },
    saveDataTable(levelRow) {
      this.$set(levelRow, 'editMode', false)
      levelRow.level = Object.assign({}, this.levels.find(level => level.id === levelRow.level.id))
    ...
    deleteDataTable(levelRow) {
      const index = this.arrDefinition.findIndex(e => e.level.id === levelRow.level.id)
      if (index >= 0) {
    ...
    AddToTable(currentDefinition) {
      if (!(currentDefinition.level.name === '' || currentDefinition.requirements === '')) {
        currentDefinition.level = this.levels.find(level => level.name === currentDefinition.level.name)
        let tmp = Object.assign({}, currentDefinition.level)
        this.arrDefinition.push({level: tmp, requirements: currentDefinition.requirements})
        this.updateRemainingLevels()
        this.currentDefinition.requirements = {...this.currentDefinition.level} = ''
    ...
    async addData(currentDefinition) {
      ...
      try {
        ...
        if (res.status === 201) {
          ...
          this.currentDefinition.skill.name = this.currentDefinition.requirements = {...this.currentDefinition.level} = ''
