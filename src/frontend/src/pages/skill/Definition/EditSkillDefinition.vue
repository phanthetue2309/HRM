      ...
      <div class="inputContent">
        <div class="d-flex">
          <div class="col-10">
            ...
            <el-input placeholder="Please input" v-model="currentDefinition.skill.name" class="inputName"></el-input>
          </div>
          <div class="col-2 d-flex justify-content-center align-items-center btnSaveSkill"
        ...
        <div class="p-2 border-definition">
          ...
          <div class="d-flex justify-content-center align-items-center btnHandleData"
          ...
          <table>
            ...
            <tr v-for="(definition,index) in arrSkillDefinitionEdit" :key="index">
              <td>
                <span v-if="definition.editMode">
                  <el-select v-model="definition.level.name" class="inputName">
                    <el-option
              ...
              <td class="text-left" style="white-space: pre-wrap">
                ...
                <span v-else style="white-space: pre-wrap">{{ definition.requirements }}</span>
              </td>
              <td>
                <div v-if="!definition.editMode">
                  <img :src="require('@/static/images/IconEdit.svg')" @click="editDataTable(definition)">
...
export default {
  ...
  async created() {
    ...
    this.editLevels = this.allLevels.filter(level => this.skillDefinitionEdit.data.filter(skillDefinition => skillDefinition.level.id === level.id).length === 0)
  ...
  methods: {
    async editSkill(currentDefinition) {
      try {
        const resSkill = await SkillDefinitions.updateSkill(this.skillDefinitionEdit.id, currentDefinition.skill)
    ...
    async saveDataTable(definition) {
      if (definition.isEdit === false) {
        definition.level = this.allLevels.find(level => level.name === definition.level.name)
        definition.skill = {
        ...
        try {
          ...
          if (res.status === 201) {
            ...
            const index = this.arrSkillDefinitionEdit.findIndex(e => e.id === null)
    ...
    async deleteDefinition(definition) {
      try {
        ...
        if (res.status === 204) {
          ...
          const index = this.skillDefinitionEdit.data.findIndex(e => e.id === definition.id)
          const indexArr = this.arrSkillDefinitionEdit.findIndex(e => e.id === definition.id)
    ...
    cancelEditDataTable(definition) {
      if (definition.isEdit === false) {
        const index = this.arrSkillDefinitionEdit.findIndex(e => e.isEdit === false)
        this.arrSkillDefinitionEdit.splice(index, 1)
      } else {
        this.$set(definition, 'editMode', false)
    ...
    async createDefinition() {
      if (this.arrSkillDefinitionEdit.find(definition => definition.id === null) === undefined) {
