          ...
          <template slot-scope="scope">
            <div class="d-flex justify-content-center align-items-center">
              <div class="skill-content">
                <div class="item row mb-2" v-for="(item, index) in positionData.rows[scope.$index].title_skills"
                     :key="item.id">
                    ...
                    <div class="d-flex justify-content-between rate-start">
                      <div class="d-flex align-items-center">
                        ...
                        <div v-if="!item.editMode">
                          <cite class="author">(Set by {{ item.set_by_user_name }})</cite>
                        </div>
                      </div>
                        ...
                        <template v-slot:default>
                          ...
                          <div v-else>
                            <img :src="require('@/static/images/IconCheck.svg')" class="mr-3"
                                 @click="saveDataTable(item, scope.$index, positionData.rows[scope.$index])">
                ...
                <el-form ref="skillForm" label-width="120px" :class="formIsHidden[scope.$index] ? 'activeHidden' : ''"
                  ...
                  <div class="col-md-6 d-flex  pl-3">
                    <div class="d-flex align-items-center justify-content-between rate-start">
                      <el-rate
                        @change="onChangeStar($event, scope.$index)"
                        v-model="skillForm.skillStar[scope.$index]"
                        :max="skillForm.maxStar[scope.$index]"
                      ...
                      <restricted-view :scopes="['title_skill:edit']">
                        <template v-slot:default>
                          <el-button
                            type="primary"
                            @click="addSkill(scope.$index, positionData.rows[scope.$index])"
                          >
...
export default ({
  ...
  methods: {
    ...
    async saveDataTable(currentData, row, position) {
      if (!this.skillForm.skillName[row] || !this.skillForm.skillLevel[row] || !this.skillForm.skillStar[row]) {
      ...
      const skillData = this.skillData.find(e => e.id === currentData.skill_id)
      const levelData = skillData.level.find(e => e.id === currentData.level_id)
      const data = {
        title: position.id,
        skill_definition: levelData.definition
      }
      try {
        const res = await positionSkillService.updatePositionSkill(currentData.id, data)
        if (res.status === 200) {
          this.formIsHidden = []
    ...
    async removeSkill(row, index, positionSkillId) {
      try {
        ...
        return this.$toast.error('Deleted Failed')
      }
    },
    async addSkill(row, position) {
      ...
      const checkData = position.title_skills.find(element => element.skill_id == newSkillId);

      if (checkData) {
        return this.$toast.error(responeMessage.VALIDATION.POSITION_SKILL)
      }

      const skillData = this.skillData.find(element => element.id == newSkillId);
      const levelData = skillData.level.find(element => element.id == newLevelId)
      ...
      try {
        ...
        if (responseData && responseData.status == 201) {
          ...
          return this.$toast.error('Created Failed')
        }
    ...
    async getData() {
      ...
      let ArrayUnique = _.uniqBy(skillDefinitionsData, "skill.name").map(element => element.skill)
    ...
    definitionStar(skillId, levelId) {
      const found = this.skillData.find(element => element.id == skillId);
      const star = found.level.findIndex(element => element.id == levelId) + 1;
      return star
    },
    ...
    onChangeLevel(event, index) {
      const found = this.skillData.find(element => element.id == this.skillForm.skillName[index]);
      this.skillForm.skillStar[index] = found.level.findIndex(element => element.id == event) + 1;
    },
