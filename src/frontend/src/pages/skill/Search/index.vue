  ...
  <div class="row mt-3 p-sm-3">
    <div class="col-xl-3 col-12 p-xl-0 search-form" v-if="skillDefinitions">
      ...
      <div class="p-2">
        ...
        <br/>
        <el-input v-model="currentData.nameUser" placeholder="Please fill input"></el-input>
      </div>
      <div class="p-2">
        <div>
        ...
        <div v-for="skill in skillDefinitions" :key="skill.id" class="mt-2">
          <div>
            <label class="d-flex align-items-center label-text mb-1">
              <input class="mr-2" type="checkbox" @change="changeShowingState(skill)">
              {{ skill.name }}
            </label>
            <select :ref="`skill-definition-${skill.id}`" class="select" v-show="skill.isShowing">
              <option value="-1">All</option>
              <option v-for="level in skill.level" :key="level.id" :value="level.definition">{{ level.name }}</option>
            </select>
          </div>
        </div>
        ...
        <div>
          ...
          <label class="d-flex align-items-center label-text mt-2" style="font-style:italic;">
            <input class="mr-2 mycb" type="checkbox" v-model="currentData.checkAllSkill">
            User must have all selected skills
          </label>
        </div>
      </div>
      <div class="d-flex justify-content-center align-items-center btnSearch"
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div v-for="item in skillSearch[scope.$index].data" :key="item.id"
              ...
              <div class="col-md-3" v-if="skillDefinitions">
                <el-rate
                    ...
                    :value="countStar(item.skill_definition.skill.id, item.skill_definition.level.id)"
                    class="text-left"
                >
                </el-rate>
              </div>
...
export default {
  ...
  methods: {
    ...
    async searchData() {
      ...
      this.skillDefinitions.forEach(skill => {
        if (skill.isChecked) {
          const definitionId = this.$refs[`skill-definition-${skill.id}`][0].value;
          if (definitionId === '-1') {
      ...
      const res = await skillSearch.getAll(this.page, this.page_size, queryData);
