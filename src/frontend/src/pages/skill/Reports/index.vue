...
export default {
  ...
  methods: {
    ...
    handleDataAPI(dataAPI) {
      ...
      this.uniqueLevels = _.uniqBy(dataAPI, "skill_definition.level.id").map(element => element.skill_definition.level)
    ...
    handleSkillData(dataAPI) {
      ...
      dataAPI.forEach(item => {
        const skill = this.skills.find(skill => skill.name === item.skill_definition.skill.name)
