        ...
        <el-col class="assessors" :span="14">
          ...
          <template v-for="(assessor, index) in criteria.assessor_roles">
            ...
            v-if="type_item === 'competencies'"
              ...
              :key="index"
            >
                <el-option v-for="item in getListAssessor(_idx)" :key="item" :value="item">
                </el-option>
            </el-select>
             <el-select
            v-else
  ...
  methods: {
    ...
    addAssessor(_idx) {
      if (this.type_item === "competencies") {
        if (this.criterias[_idx].assessor_roles.length === this.titles.length + 2) {
          this.$nextTick(() => {
    ...
    getListAssessor(_idx) {
      ...
      let ls = this.criterias[_idx].assessor_roles.map(({assessor_role})=> assessor_role);
