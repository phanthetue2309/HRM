  ...
  <div class="mt-3 mb-5">
    <div class="actions">
      <restricted-view :scopes="['evaluation_template:create']">
        <el-button id="btn-add-template" type="primary" icon="el-icon-circle-plus" @click="addNewEvaluationTemplate">
          New Evaluation Form Template
        </el-button>
      </restricted-view>
    </div>
    <el-table highlight-current-row :data="template.results" style="width: 100%" header-cell-class-name="bg-header-table"
      border>
      ...
      <el-table-column sortable prop="created_at" :formatter="formatter" label="Create at" width="130" align="center">
      </el-table-column>
      <el-table-column sortable prop="updated_at" :formatter="formatter" label="Update at" width="130" align="center">
      </el-table-column>
      ...
      <el-table-column prop="status" label="Active" width="80" align="center">
        <template slot-scope="scope">
          <restricted-view :scopes="['evaluation_template:edit']">
            <el-switch active-color="#25c9d0" inactive-color="#ff4949" v-model="scope.row.status"
      ...
      <el-table-column label="Action" width="120">
        <template slot-scope="scope">
          <restricted-view :scopes="['evaluation_template:edit']">
          <img :src="require('@/static/images/IconEdit.svg')" class="ml-2" @click="redirectToEditPage(scope.row.id)"
            title="Edit Probation Template" />
          <img :src="require('@/static/images/IconDelete.svg')" class="ml-1" @click="deleteRequest(scope.row.id)"
            title="Delete Probation Template" />
          </restricted-view>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog title="Delete" :visible.sync="dialogVisible" width="30%" class="dialog">
  ...
  export default {
    ...
    methods: {
      ...
      addNewEvaluationTemplate() {
        window.location.href = "/evaluation-form-templates/new-evaluation-form-template";
      },
      ...
      async asyncDataActive() {
        ...
        if (response && response.status === 200 && response.data) {
          this.setEvaluationTemplates(response)
        }
      },
      ...
      async deleteItem() {
        if (this.delete_id !== "") {
          ...
          let res = await probationService.deleteEvaluationTemplate(this.delete_id)
          if (res.status === 204) {
            this.$nextTick(() => {
              this.$toast.success("Your evaluation template was deleted successfully");
            });
            await this.asyncDataActive();
          } else {
