...
describe("criteriaItemComponent.vue", () => {
  it("renders props.criterias props.isAdd props.type_item when passed", () => {
    ...
    const wrapper = shallowMount(CriteriaItem, {
...
describe("addNewEvaluationTemplate.vue", () => {
  ...
  it("change value when submitted", async () => {
    ...
    wrapper.setData({ formEvaluationTemplate: { "name": "Evaluation Template Builder", "office": 1, "competencies": [{ "competence": "com 1", "assessor_roles": [{ "assessor_role": "Self-evaluation" }] }], "overall_comments": [{ "term": "overall", "assessor_roles": [{ "overall_comment_role": "Self-evaluation" }] }] } })
...
describe("myEvaluationTemplate.vue", () => {
  ...
  it("redirect to /new-evaluation-form-template/ when clicking on add button", async () => {
    ...
    expect(window.location.href).toBe('/evaluation-form-templates/new-evaluation-form-template')
  })
})
