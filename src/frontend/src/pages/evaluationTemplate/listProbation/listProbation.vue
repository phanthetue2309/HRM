    ...
    <el-card>
      <div class="btn-new">
        <restricted-view :scopes="['probation:create']">
          <el-button class="btn-new-probation" type="primary" icon="el-icon-circle-plus" @click="addNewProbation">
            New Evaluation
          </el-button>
        </restricted-view>
      </div>
      <div class=" office-export">
        <div class="d-flex">
          <div class="d-flex align-items-center">
            <span>Office: </span>
            <el-select v-model="search.office" placeholder="Office" clearable @change="handleSearch">
              <el-option
            ...
            <el-select v-model="search.type" placeholder="Type Evaluation" clearable @change="handleSearch">
              <el-option
          ...
          <el-button size="medium" @click="handleSearch" type="primary" icon="el-icon-search">Search</el-button>
        </div>
        <div>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.editMode">
              <el-input v-model="scope.row.employee.profile.name"
                        :placeholder="scope.row.employee.profile.name === '' ? 'Full Name*' : scope.row.employee.profile.name">
              </el-input>
            </div>
            <div v-else>
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.editMode">
              <el-input v-model="scope.row.employee.profile.join_date" placeholder="Start Date*"></el-input>
            </div>
            <div v-else>
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.editMode">
              <el-input v-model="scope.row.probation_end_date" placeholder="End Date*"></el-input>
            </div>
            <div v-else>
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.editMode">
              <el-input v-model="scope.row.remind" placeholder="Remind*"></el-input>
            </div>
            <div v-else-if="scope.row.remind === reminder"> {{ scope.row.remind }}</div>
        ...
        >
          <template slot-scope="scope">
            <div v-if="scope.row.editMode">
              <el-input v-model="scope.row.type_name" placeholder="Type Evaluation*"></el-input>
            </div>
            <div v-else>
              {{ scope.row.type_name }}
            </div>
        ...
        >
          <template slot-scope="scope">
            <img :src="require('@/static/images/IconEdit.svg')" class="ml-2" @click="showViewDetailProbation(scope.row)"
                 title="Edit Probation List"/>
            <restricted-view :scopes="['probation:destroy']">
              <img :src="require('@/static/images/IconDelete.svg')" class="ml-1" @click="deleteProbation(scope.row.id)"
...
export default {
        ...
        cancelButtonText: 'No'
          ...
          .then(async () => {
                ...
                if (res.status === 204) {
                  ...
                  this.$toast.error('Delete failure')
                }
              }
          )
          .catch(() => {
            this.$message({
              type: 'warning',
              message: this.$toast.warning('You canceled')
            })
          });
    },
  ...
  watch: {
    query: {
      ...
      handler: function () {
        this.getListProbations()
        this.$router.push({path: '/evaluations', query: this.query}).catch(err => err)
      }
    }
  }
}
