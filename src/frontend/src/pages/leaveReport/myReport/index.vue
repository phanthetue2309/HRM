...
export default {
  ...
  methods: {
    ...
    async getData() {
      this.monthInYear = []
      const res = await StatisticDateOffService.getByUser(this.yearChoose)
    ...
    getDataByMonthYear() {
      ...
      const monthHoliday = {
        name: 'Holidays',
        data: [this.getDetailData(this.monthChoose, 'holidays'), 0]
      }
      this.series = []
      this.series.push(monthHoliday)
      for (const header in this.headers) {
        // find number date off of leave type
        const temp = this.getDetailData(this.monthChoose, this.headers[header].value)
        if (temp) {
          const leftsDay = {
    ...
    getDetailData(month, value) {
      return this.leaveUser.find((date) => date.month === parseInt(month))[value]
    },
    ...
    async fetchDataForExportExcel() {
      const data = await StatisticDateOffService.getByAdmin(this.monthChoose, this.yearChoose, 1, 12, this.name, this.email, '')
