    ...
    <div class="general-infor">
      <el-card class="box-card mb-4 card-detail">
        <div slot="header" class="clearfix">
          ...
          <el-button class="edit-button mb-3" type="text" v-if="!isEditing" @click="isEditing = true">
            <restricted-view :scopes="['personal_skill:edit']">
              <template v-slot:default>
                <img :src="require('@/static/images/IconCardEdit.svg')"
                     class="edit-icon"/>
              </template>
            </restricted-view>
          </el-button>
          <el-button class="edit-button" type="text" v-if="isEditing" @click="isConfirming = true">
            <img :src="require('@/static/images/IconCardSave.svg')"
                 class="edit-icon"/>
          </el-button>
        </div>
        <el-form class="d-flex justify-content-center" v-if="isEditing">
          <el-form-item class="mr-2 ml-2">
            <el-select style="width:100%"
              ...
              <el-option v-for="skill in skillDefinitions"
                         :key="skill.id"
                         :value="skill.id"
                         :label="skill.name">
              </el-option>
          ...
          <el-form-item class="mr-2 ml-2 d-flex align-items-center">
            <el-rate @change="onChangeStar($event)"
                     :max="currentInput.maxStar"
                     v-model="currentInput.star"
                     :disabled="!currentInput.skill_id">
            </el-rate>
          </el-form-item>
          <div class="d-flex justify-content-center align-items-center btnAddSkill"
               @click="AddCurrentSkill(currentInput)">
            <span class="ml-1">Add Skill</span>
          </div>
        </el-form>
        <el-table :data="currentSkill"
          ...
          <el-table-column label="Level">
            <template slot-scope="scope">
              <div class="mobile-screen d-flex align-items-center mb-2 mt-2 justify-content-center"
          ...
          <el-table-column label="Rate">
            <template slot-scope="scope">
              <div class="mobile-screen d-flex align-items-center justify-content-center mb-2 mt-2"
                   v-for="item in currentSkill[scope.$index].data"
                   :key="item.id">
                <el-rate :disabled="!isEditing"
          ...
          <el-table-column label="Action" v-if="isEditing">
            <template slot-scope="scope">
              <div class="mobile-screen d-flex align-items-center justify-content-center mb-2 mt-2"
                ...
                <restricted-view :scopes="['personal_skill:edit']">
                  <template v-slot:default>
                    <div :class="[{active : !item.isMineVote},'btnUnvote']"
...
export default {
  ...
  methods: {
    ...
    async voteSkill(currentSkill, levelId, star) {
      const skill = this.skillDefinitions.find((e) => e.id == currentSkill.skill_id);
                ...
                .map((value, keySkill) => {
                  let groupByLevel = _.chain(value)
                      ...
                      .map((val, key) => ({
                        ...
                        isMineVote: (val.find((el) => el.id == this.voter_user) !== undefined),
                        max_title_weight: Math.max(
                            ...val.map((el) => el.voter_title_weight)
                        ),
                        voters: _.orderBy(val.map((el) => ({
                          id: el.voter,
                          name: el.voter_name,
                          skill_vote: el.id,
                        })), 'voter_title_weight', 'desc'),
                      }))
                      .value();
    ...
    getMyVote(skillId) {
      ...
      const level = skill.data.find(
          (level) => level.voters.find((el) => el.id === this.voter_user) !== undefined
      );
      return level == undefined ? undefined : level.voters.find((el) => el.id == this.voter_user);
    },

    syncVotedField() {
      this.currentSkill.forEach((skill) => {
        skill.data.forEach(level => {
          level.isMineVote = (level.voters.find((el) => el.id == this.voter_user) !== undefined);
        })
      })
    },
    async removeMyVote() {
      ...
      if (res.status === 204) {
        ...
        this.currentSkillToDelete.skillIndex = this.currentSkillToDelete.levelIndex = null
    ...
    isEmptySkill(skillIndex) {
      return this.currentSkill[skillIndex].data.find((level) => level.voters.length !== 0) === undefined;
    },
