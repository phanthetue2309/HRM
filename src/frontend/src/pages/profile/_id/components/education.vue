      ...
      <el-card class="box-card mb-4 card-detail">
        <div slot="header" class="clearfix">
          ...
          <el-button
            ...
            type="text"
            @click="editData"
            v-if="!isEditing && checkOwnerOrHasScope('user:edit_private_user_information_list')"
          >
            <img
        ...
        <div>
          <div class="d-flex" :class="{active: hiddenFormAdd}">
            <div class="col-2 p-0 mt-2 mb-2 ml-3 mr-3">
              <el-input v-model="formData.school" placeholder="Your university name"/>
            ...
            <div class="col-2 p-0 ml-3 mt-2 mb-2">
              <el-input v-model="formData.additional_notes" placeholder="Notes"/>
            </div>
            <div class="text-center p-0 ml-3 m-2" style="width: 100%">
              <el-button type="primary" @click="createEducation()"
          ...
          >
            <el-table-column prop="school" label="School Name">
              <template slot-scope="scope">
                <el-input v-model="scope.row.school" :disabled="!scope.row.editMode"/>
              </template>
            </el-table-column>
            <el-table-column prop="degree" label="Degree">
              <template slot-scope="scope">
                <el-input v-model="scope.row.degree" :disabled="!scope.row.editMode"/>
              </template>
            </el-table-column>
            <el-table-column prop="field_of_study" label="Major">
              <template slot-scope="scope">
                <el-input v-model="scope.row.field_of_study" :disabled="!scope.row.editMode"/>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="additional_notes" label="Notes">
              <template slot-scope="scope">
                <el-input v-model="scope.row.additional_notes" :disabled="!scope.row.editMode"/>
              </template>
            </el-table-column>
            <el-table-column label="Action" width="150">
              <template slot-scope="scope" v-if="isEditing">
                <div class="text-center"
                     v-if="!scope.row.editMode && checkOwnerOrHasScope('user:edit_private_user_information_list')">
          ...
          <el-dialog :visible.sync="dialogDelete" title="Confirm Delete">
            <slot>
              ...
              <div class="text-center">
                <el-button type="danger" class="mt-3" @click="deleteEducation()">
                  Delete
                </el-button>
              </div>
            </slot>
...
export default {
  ...
  methods: {
    checkOwnerOrHasScope(scope) {
      return this.userId === this.$store.state.scope.tokenInfo.sub ||
    ...
    async createEducation() {
      try {
        this.userEducations.push((await EducationService.create(this.userId, this.formData)).data)
        this.formData = {}
        this.dialog = false
    ...
    async updateEducation(userEducation) {
      try {
        let tempEducation = (await EducationService.update(this.userId, userEducation)).data
    ...
    async deleteEducation() {
      try {
        ...
        if (res.status >= 200 && res.status < 400) {
          this.userEducations.splice(this.userEducations.indexOf(this.educationToDelete), 1)
