<template>
  <div>
    <div class="mb-3">
      We've detected info and assessors in account HRM. There may be some names you write in file excel that are not the
      same as
      in hrm.
      <br>
    ...
    >
      <el-row class="d-flex align-items-center">
        ...
        <el-col :span="9" v-if="(index === 0 || index === 3)">
          <el-col v-if="info.check_employee_id">
            Ok
          </el-col>
          <el-col v-else>
            No
            <el-select v-model="info.employee_id" class="ml-4" placeholder="Employee Name" style="width: 200px"
    ...
    >
      <el-row class="d-flex align-items-center">
        ...
        <el-col :span="3" v-if="overall.old_assessor">
          Ok
        </el-col>
        <el-col :span="7" v-else>
          No
          <el-select v-model="overall.assessor" class="ml-4" placeholder="Employee Name" style="width: 200px"
...
export default {
  ...
  methods: {
    changeAssessor(assessorId, index) {
      const newOverall = this.newOveralls[index].assessor_name = this.employeesData.find((item) => item.id === assessorId)?.profile.name
      this.newOveralls.slice(index, 1, ...newOverall)
      this.$emit("newOveralls", this.newOveralls);
    },
    changeEmployee(employeeId, index) {
      const newInfos = this.newInfos[index].employee_id = this.employeesData.find((item) => item.id === employeeId)?.id
