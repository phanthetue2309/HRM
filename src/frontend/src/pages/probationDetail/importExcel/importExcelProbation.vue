<template>
  <div class="import-excel-container">
      ...
      <div class="probation-import">
        <div class="d-flex ">
          ...
          <div class="d-flex align-items-center choose-template">
            <el-form-item class="ml-3" label="Choose Template" prop="evaluationTemplate">
              <el-select v-model="evaluationTemplate" placeholder="Choose Template" style="width: 400px" size="medium"
        ...
        <div>
          <el-upload
              ...
              style="float: right"
          >
            <el-button style="font-size: 15px; font-weight: bold; float: right" icon="el-icon-document-add"
                       type="primary">Import from Excel
            </el-button>
          </el-upload>
        </div>
      </div>
    ...
    <div class="bg-light">
      <el-card v-if="listInfo.length && listCompetence.length && listOverall.length">
        <div class="row mt-4">
          ...
          <div class="col-9 col-lg-9 col-xl-9 mt-2">
            ...
            <el-form
                ...
                style="width: 80%"
            >
              <div v-for="(info, index) in listInfo"
                   :key="index">
                <el-form-item :label="info.title">
                  <el-input :value="info.value" disabled>
                  </el-input>
                </el-form-item>
              </div>
            </el-form>
        ...
        >
          ...
          <div v-for="(col,index) in listExcelCompetenceRole" :key="index" class="d-none">
        ...
        >
          ...
          <el-table-column prop="comment" label="Comments" width="350" align="center">
            <template slot-scope="scope">
              <span style="word-break: break-word;">{{ scope.row.comment }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="assessor" label="Assessor" width="350" align="center">
            <template slot-scope="scope">
              <span style="word-break: break-word;">{{ scope.row.assessor }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getListOverallRole(templateId) {
      const res = await ProbationService.getEvaluationOverallsAssessorRole(templateId)
    ...
    confirmTemplate(excelData) {
      ...
      this.$msgbox({
        ...
        else return this.$toast.error("The selected template doesn't match with the imported excel");
      })
    },
    ...
    confirmAssessor(excelData) {
      const overallAssessors = excelData.slice(this.overallIndex + 1, this.overallIndex + 1 + this.roleOverallColLength).map((overall) => ({
        overall: overall[0],
        old_assessor: this.getAssessorId(overall, this.employeesData, this.assessorOverallColIndex),
        assessor_name: overall[this.assessorOverallColIndex],
        assessor: this.getAssessorId(overall, this.employeesData, this.assessorOverallColIndex),

        role: overall[this.roleOverallColIndex],
      }))
      const infos = this.removeEmptyExcelData.slice(this.infoIndex, this.infoIndex + 6).map((row) => ({
        title: row[0],
        value: row[1],
    ...
    showImportExcel(excelData) {
      this.listInfo = this.removeEmptyExcelData.slice(this.infoIndex, this.infoIndex + 6).map((row) => ({
        title: row[0],
        value: row[1]
      }))

      this.listOverall = excelData.slice(this.overallIndex + 1, this.overallIndex + 1 + this.roleOverallColLength).map((row, index) => ({
        name: row[0],
        score: row.slice(1, 6).findIndex((item) => item) + 1,
        comment: String(row[this.assessorOverallColIndex - 1]),
        assessor: this.newOveralls[index].assessor ? this.newOveralls[index].assessor_name : "",
        role: row[this.roleOverallColIndex],
      }));

      let competence = excelData.slice(this.competenceIndex + 1, this.overallIndex)
      ...
      for (let i = 0; i < newCompetence.length; i++) {
        ...
        const combineCompetence = this.listExcelCompetenceRole.reduce((obj, item, index) => ({
          ...obj,
          [item]: {
            score: evaluateArr[index]?.slice(0, 5).findIndex((item) => item) + 1,
      ...
      this.listOutcome = excelData.slice(this.outcomeIndex + 1, this.outcomeIndex + 1 + 4).map((row) => ({
        title: row[0],
    ...
    getAssessorId(overall, employeesData, index) {
      return employeesData.find((item) => item?.profile.name === overall[index])?.id
    },
