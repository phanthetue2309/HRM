<template>
  <div>
    ...
    <div>
      <el-row :gutter="20">
        <el-col :span="7" class="mt-2">Competencies have {{competenceRoles.length}} roles:</el-col>
        <el-col :span="6">
        </el-col>
        <el-col :span="11">
          <div class="cols">
    ...
    <div>
      <el-row :gutter="20">
        <el-col :span="7" class="mt-2">Overall comments have {{overallRoles.length}} roles:</el-col>
        <el-col :span="6">
        </el-col>
        <el-col :span="11">
          <div class="cols">
    ...
    <div>
      <el-row :gutter="20">
        <el-col :span="7" class="mt-2">Competencies have {{roleCompetenceLength}} roles:</el-col>
        <el-col :span="6">
    ...
    <div>
      <el-row :gutter="20">
        <el-col :span="7" class="mt-2">Overall comments have {{roleOverallLength}} roles:</el-col>
        <el-col :span="6">
    ...
    <br>
    The selected template <span style="color: blue">{{(isCheckTemplate) ? '' : "doesn't"}} match</span>  with the imported excel. <i>Select ok if it is correct, if not, choose cancel then import it again. </i>
...
export default {
  ...
  computed: {
    ...
    overall() {
      const overall = this.headers.slice(this.overallValue + 1, this.outcomeValue).map(item => item[this.roleOverallColIndex])
    ...
    isCheckTemplate() {
      this.$emit("checkTemplate", this.isCheckOverall && this.isCheckCompetence);
  ...
  methods: {
    ...
    getIndexOption(row, rowIndex) {
      if (!this.competenceValue && this.checkHeader(row, "Competencies")) this.competenceValue = rowIndex - 2
      ...
      else if (!this.outcomeValue && this.checkHeader(row, "Outcome")) this.outcomeValue = rowIndex
    },
    isSameTwoArray(firstArr, secondArr) {
      return _.isEmpty(_.xor(firstArr, secondArr)) && firstArr.length === secondArr.length
    },
    checkCompetence(competence) {
      ...
      this.isCheckCompetence = this.isSameTwoArray(cleanCompetence, this.competenceRoles)
    },
    checkOverall(overall) {
      ...
      this.isCheckOverall = this.isSameTwoArray(cleanOverall, this.overallRoles)
    }
  },
};
</script>
