...
export default {
  ...
  methods: {
    ...
    async getReminders() {
      const res = await ProbationReminderService.getReminders(this.$route.params.id)
      if (res.data && res.data.length) {
        this.reminders = [...res.data]
        const reminderDates = this.reminders.map((item) => new Date(item.reminder_date))
        this.$emit('maxReminderDate', moment(new Date(Math.max(...reminderDates))).format('YYYY-MM-DD'))
      } else {
    ...
    isExistsReminderDate() {
      let unique = [...new Set(this.reminders.map(item => item.reminder_date))];
      return unique.length < this.reminders.length;
    },
    async saveReminder() {
      if (!this.isExistsReminderDate()) {
        if (!this.isCreated && this.reminders.length) {
          await ProbationReminderService.create(this.newReminders, this.$route.params.id)
              .then((res) => {
                if (res.status === 201) {
                  this.$nextTick(() => {
          ...
          this.isShow = false
          await ProbationReminderService.update(this.reminders, this.$route.params.id)
              .then((res) => {
                if (res.status === 200) {
                  this.$nextTick(() => {
                    this.$toast.success('Set reminder time successfully')
