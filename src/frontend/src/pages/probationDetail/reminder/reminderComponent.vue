    ...
    >
      ...
      <el-col :span="3">
        <el-input-number
            ...
            @change="changeReminderDate(listDuration[index], listTimeUnit[index], index)"
            :min="1"
            :max="10"
            defau
        >
...
export default {
  ...
  methods: {
    changeReminderDate(duration, timeUnitKey, index) {
      this.reminders[index].reminder_date = this.getReminderDate(this.timeUnits[timeUnitKey].value * duration)
    },
    ...
    getListDuration(reminders) {
      ...
      for (let i = 0; i < reminders.length; i++) {
        let subDays = new Date(this.probationEndDate).getDate() - new Date(reminders[i].reminder_date).getDate()
