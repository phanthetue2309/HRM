      ...
      <div class="row m-4">
        ...
        >
          ...
          <el-button type="danger" @click="back()">
            ...
            <span class="ml-2">Discard</span><i class="fa-solid fa-arrow-left-long-to-line"></i>
          </el-button>
        </div>
      </div>
      <el-card>
        <div class="row mt-4">
          ...
          <div class="col-9 col-lg-9 col-xl-9 mt-2">
            ...
            >
              ...
              <el-input :value="jobTitle" placeholder="Job Title" disabled></el-input>
            </el-form-item>
              ...
              <el-form-item label="End date of self-evaluation" prop="selfEvaluationEndDate">
                <div>
              ...
              <el-form-item class="choose-template-select" label="Choose Template">
                <el-select v-model="evaluationTemplate" placeholder="Choose Template" @change="changeTitle($event)"
                           disabled>
                  <el-option
        ...
        >
          <el-table-column class="competence-header" fixed
                           label="Competencies" width="450">
            <template slot-scope="scope">
              <span class="break-work">{{ scope.row.evaluation_template_competence.competence }}</span>
            </template>
          </el-table-column>
          <div v-for="(col,index) in competenceAssessorRoles" :key="index" style="display: none">
            <el-table-column :prop="col" :label="col">
              <el-table-column :render-header="(h,obj) => renderHeader(h,obj,col)">
                <el-table-column label="Score" align="center" width="250">
                  <template slot-scope="scope">
                    <div v-if="((scope.row.evaluation_template_competence_assessor_role)[col])">
                      <el-radio-group
                          v-model="((scope.row.evaluation_template_competence_assessor_role)[col]).score"
                          :disabled="checkIsAdmin ? !isEdit : checkEvaluation(((scope.row.evaluation_template_competence_assessor_role)[col]).assessor_id)"
                      >
                ...
                <el-table-column prop="comment" label="Comments" width="340" align="center">
                  <template slot-scope="scope">
                    <div v-if="((scope.row.evaluation_template_competence_assessor_role)[col])">
                      <el-input
                          type="textarea"
                          placeholder="Please input"
                          v-model="((scope.row.evaluation_template_competence_assessor_role)[col]).comments"
                          :disabled="checkIsAdmin ? !isEdit : checkEvaluation(((scope.row.evaluation_template_competence_assessor_role)[col]).assessor_id)"
                          class="break-work"
                      >
                      </el-input>
        ...
        >
          <el-table-column prop="overCmt" label="Overall comments" width="450" fixed>
            <template slot-scope="scope">
              <span style="word-break: break-word;">{{ scope.row.evaluation_template_overall_comment.term }}</span>
            </template>
          </el-table-column>
          <el-table-column label="Score" align="center" width="250">
            <template slot-scope="scope">
              <el-radio-group
                  ...
                  :disabled="checkIsAdmin ? !isEdit : checkEvaluation(scope.row.assessor_id)"
              >
                <el-radio class="custom-radio" :label="1"></el-radio>
                <el-radio class="custom-radio" :label="2"></el-radio>
                <el-radio class="custom-radio" :label="3"></el-radio>
          ...
          <el-table-column prop="comment" label="Comments" width="350" align="center">
            <template slot-scope="scope">
              <el-input
                  placeholder="Please input"
                  type="textarea"
          ...
          <el-table-column prop="assessor" label="Assessor" width="350" align="center">
            <template slot-scope="scope">
              <el-select
                  v-model="scope.row.assessor_id"
                  placeholder="Employee Name"
                  :disabled="(scope.row.evaluation_template_overall_comment_assessor_role.overall_comment_role === lineManagerRole) ? true : permitIfOneOfAllScope(['probation:create'])">
                <el-option
                    v-for="item in filterEmployee(scope.row.evaluation_template_overall_comment_assessor_role.overall_comment_role)"
                    :key="item.index"
                    :label="item.profile.name"
                    :value="item.id"
                >
          ...
          <el-table-column label="Role" width="350" align="center">
            <template slot-scope="scope">
              <el-input
                  placeholder="Please input"
                  v-model="scope.row.evaluation_template_overall_comment_assessor_role.overall_comment_role"
                  disabled
              >
              </el-input>
            </template>
  ...
  methods: {
    ...
    checkEvaluation(assessorId) {
      ...
      if (assessorId === this.userId) {
        if (assessorId === this.employeeId || this.isSelfEvaluated || today > this.selfEvaluationEndDate) return false
      }
      return true
    },
    disableMask() {
      ...
      let element = createElement('div', [
        createElement('el-select', {
          props: {
            ...
            disabled: role === this.lineManagerRole ? true : this.permitIfOneOfAllScope(['probation:create']),
          },
          on: {
            input: (value) => {
            ...
            change: (value) => {
              this.competenceData.forEach(item => {
                if (role in item.evaluation_template_competence_assessor_role) {
                  item.evaluation_template_competence_assessor_role[role].assessor_id = value
                }
              })
            }
          }
    ...
    filterEmployee(role) {
      if (role) {
        if (role === this.selfRole) {
          return this.employeesData.filter(el => el.id === this.probationDetail.employee.id)
        }
        if (role === this.lineManagerRole) {
          return this.filteredEmployeeExcludeUserData
        }
        return this.employeesData.filter(el => (el.title) ? (el.title.title?.trim() === role.trim()) : false)
      }
    },
    formatDate(value) {
      if (value) {
    ...
    async getCurrentProbation() {
      ...
      this.competenceData = await this.getCompetenceDataCombineAccessorRole(res.data.competencies)
    ...
    async getListEvaluationAssessorRole(templateId) {
      const res = await ProbationService.getEvaluationCompetencesAssessorRole(templateId)
      this.assessorCompetence = Object.assign({}, ...res.data.map(key => ({[key]: {'assessor': ""}})));
      ...
      if (competenceData.length) {
        competenceData.forEach((item) => {
          let assessorRoleItem = {
            ...
            "id": item.id,
            "competence_role": item.evaluation_template_competence_assessor_role.assessor_role
          }
          let changeKey = item.evaluation_template_competence_assessor_role.assessor_role
          let existingArr = output.filter(function (v, i) {
            return (
                v.evaluation_template_competence.id === item.evaluation_template_competence.id
            );
          });
          if (existingArr.length) {
            let existingIndex = output.indexOf(existingArr[0]);
            output[existingIndex].evaluation_template_competence_assessor_role[`${changeKey}`] = assessorRoleItem
            output[existingIndex] = _.pick(output[existingIndex], ['evaluation_template_competence_assessor_role', 'evaluation_template_competence']);
            output[existingIndex].evaluation_template_competence_assessor_role = _.pickBy(output[existingIndex].evaluation_template_competence_assessor_role, _.isPlainObject);
          } else {
            item.evaluation_template_competence_assessor_role[`${changeKey}`] = assessorRoleItem
            item = _.pick(item, ['evaluation_template_competence_assessor_role', 'evaluation_template_competence']);
            item.evaluation_template_competence_assessor_role = _.pickBy(item.evaluation_template_competence_assessor_role, _.isPlainObject);
            output.push(item);
          }
          if (Object.keys(this.assessorCompetence).length) {
            this.assessorCompetence[`${changeKey}`]['assessor'] = assessorRoleItem.assessor?.id
          }
        });
      }
    ...
    async changeCompetenceStructure(competencies) {
      ...
      if (competencies.length) {
        competencies.forEach((item) => {
          let myData = {}
          myData = Object.keys(item['evaluation_template_competence_assessor_role']).map(key => {
    ...
    async updatedData() {
      this.probationDetail.competencies = await this.changeCompetenceStructure(this.competenceData)
      ...
      if (new Date(this.endDay).getTime() < new Date(this.maxReminderDate).getTime())
        return this.$toast.error("The probation end date is not smaller than reminder dates")
      await ProbationService.update(updatedProbationObj, this.$route.params.id)
          .then((res) => {
            if (res.status === 200) {
              this.$nextTick(() => {
                this.$toast.success('Update new probation successfully')
              })
