<template>
  <div>
    ...
    <div class="">
      <FilterBox @getData="getData" @loadData="loadData" :page_size="this.page_size" ref="filter"/>
      <el-table :data="employeesData.rows" stripe header-cell-class-name="bg-header-table" style="width: 100%;">
        <el-table-column
          prop="profile.image"
          label="Avatar"
          cell-click="`/profile/`"
        ...
        <el-table-column prop="profile.name" label="Name" align="center" width="180">
          <template v-slot:default="table">
            <router-link
              :to="`/profile/${table.row.id}`"
        ...
        <el-table-column prop="title" label="Position">
          <template v-slot:default="table">
            <div style="text-align: center;">
              {{ !table.row.title ? "" : table.row.title.map(e=>e.title).join(", ") }}
            </div>
          </template>
        </el-table-column>
        ...
        <restricted-view :scopes="['user:view_private_user_information_list']">
          <template v-slot:default>
            <el-table-column prop="profile.birth_day" label="Birthday">
            </el-table-column>
        ...
        <el-table-column prop="profile.join_date" label="On board at Paradox/Olivia" align="center">
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="Import File" :visible.sync="dialogImportFile">
      <ImportFileModal :dialogImportFile.sync="dialogImportFile"/>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  watch: {
    dialogInvite: function () {
      this.getDataFilterOption(1, this.page_size);
    },
    dialogImportFile: function () {
      this.getDataFilterOption(1, this.page_size);
    },
  },
  async created() {
