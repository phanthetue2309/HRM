    ...
    <div class="row">
      <div class="col-12">
        <div class="border-taskbar p-2">
          ...
          <div class="col-xl-2 d-flex justify-content-center align-items-center position-relative mt-3 mt-xl-0">
            <div
            ...
            <el-button v-show="$store.state.scope.tokenInfo.scope.includes('office:edit')" type="primary"
                       v-else
                       size="medium"
                       icon="el-icon-plus"
                       @click="addData()"
            ...
            <template slot-scope="scope">
              <restricted-view :scopes="['office:edit']">
                <template v-slot:default>
                  <img :src="require('@/static/images/IconEdit.svg')" class="cursor-button mr-1"
                       @click="editData(scope.$index)">
                  <img :src="require('@/static/images/IconDelete.svg')" class="cursor-button ml-1"
                       @click="deleteData(scope.$index)"></template>
              </restricted-view>
            </template>
  ...
  methods: {
    ...
    async addData() {
      try {
        ...
        const responseData = await holidayService.create(this.officeId, dataForm)
        ...
        let errorContent = error.response.status == 400 && error.response.data ? error.response.data : 'Created Failed'
    ...
    async saveData() {
      try {
        ...
        if (oldData.repeat && (
          ...
          endDate.setFullYear(oldData.start_year + (endDate.getFullYear() - startDate.getFullYear()))
          ...
          responseData = await holidayService.update(this.officeId, this.formData.id, dataForm)
        } else {
          dataForm.append('start_date', this.formData.dates[0])
          dataForm.append('end_date', this.formData.dates[1])
          dataForm.append('office', this.officeId)
        ...
        let errorContent = error.response?.status == 400 ? error.response?.data : 'Updated Failed'
        this.$toast.error(errorContent)
      }
    },
    ...
    async deleteData(index) {
      try {
        ...
        const responseData = await holidayService.delete(this.officeId, holidayId)
        if (responseData.status == 204) {
          this.$toast.success('Deleted Successfully');
          this.dataTable.splice(index, 1)
        }
      } catch (error) {
        let errorContent = error.response.status == 400 && error.response.data ? error.response.data : 'Deleted Failed'
        this.$toast.error(errorContent)
      }
    }
  },
