<template>
  <div>
      ...
      <el-col :xl="3" :lg="4" :sm="12" v-for="day in daysOfWeek" :key="day.dow">
        <el-card shadow="always" class="text-center mt-2">
          ...
          <div v-if="typeof(sessions[day.dow]) !== 'undefined'">
            <div v-for="item in sessions[day.dow].data" :key="item.id"
                 :id="!showAddSession ? 'session-item' : 'session-item-unHover'">
              <div @click="editData(item)">{{ item.start_time.slice(0, 5) }} - {{ item.end_time.slice(0, 5) }}</div>
              <img @click="deleteData(day.dow, item.id)" :src="require('@/static/images/IconCancel.svg')"
                   class="img-delete">
            </div>
          </div>
          <div class="d-flex justify-content-end" v-if="!showAddSession">
            <img @click="addData(day)" :src="require('@/static/images/IconAdd.svg')" class="img-add">
          </div>
        </el-card>
      </el-col>
    </el-row>
    <el-dialog
        ...
        destroy-on-close
    >
      <addSession :sessions="sessions" :dayAddSession="dayAddSession" @update-data="emitData"/>
    </el-dialog>
    <el-dialog
  ...
  methods: {
    ...
    async deleteData(dayDow, session_id) {
      try {
        ...
        if (res.status === 204) {
          const indexSessions = this.sessions[dayDow].data.findIndex(e => e.id === session_id)
          const indexSessionUpdate = this.sessionUpdate.findIndex(e => e.id === session_id)
