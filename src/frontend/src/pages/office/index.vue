        ...
        <template v-if="node.isEdit">
          <div class="row">
            ...
            <div class="col-12 col-md-2 mt-2 mt-md-0">
              <el-button size="mini" type="primary" @click="handleInput(node, data)">Save</el-button>
              <el-button size="mini" type="info" @click="cancel(node, data)">Cancel</el-button>
            </div>
          </div>
        </template>
        <template v-else>
          <div>
            <span class="d-block title-tree" v-if="!data.isOfficeChild">
              ...
              <font-awesome-icon
                ...
                class="option-icon"
                v-else-if="!data.isOfficeChild && node.expanded && !data.isChildrenDepartments"
              />
              <font-awesome-icon
                :icon="['fas', 'users']"
                class="option-icon"
              ...
              />
              {{ node.label }} : {{ node.data.manager_name}} - {{ node.data.manager_title}}
            </span>
            <span class="d-block title-tree" v-else>
              <router-link :to="{ name: 'OfficeInformation',  params: { id: data.id }}">
                <img :src="require('@/static/images/OfficesIcon.svg')" class="option-icon" />
                {{ node.label }} : {{ node.data.manager_name}} - {{ node.data.manager_title}}
              </router-link>
            </span>
            <template v-if="node.isAdd">
              <div class="row">
                  ...
                  <div class="row">
                    <div class="col-12 col-md-4 mt-1 mt-md-0">
                      <el-input
                        ...
                        size="mini"
                        class="field-input"
                        :placeholder="!data.createDepartment ? data.createOffice ? 'Office Name' : 'Group Name' : 'Team Name'"
                      ></el-input>
                    </div>
                    <div class="col-12 col-md-4 mt-1 mt-md-0">
                      <el-select
                ...
                <div class="col-12 col-md-2 mt-2 mt-md-0">
                  <el-button size="mini" type="primary" @click="append(node, data)">Save</el-button>
                  <el-button size="mini" type="info" @click="cancel(node, data)">Cancel</el-button>
                </div>
              </div>
            </template>
          ...
          <restricted-view :scopes="['office:edit']">
            <template v-slot:default>
              <span>
                ...
                >
                  <img :src="require('@/static/images/AddOffice.svg')" class="option-icon" />
                </el-button>
                ...
                >
                  <font-awesome-icon :icon="['fas', 'folder-plus']" class="option-icon"></font-awesome-icon>
                </el-button>
                ...
                >
                  <font-awesome-icon :icon="['fas', 'user-plus']" class="option-icon"></font-awesome-icon>
                </el-button>
                ...
                <el-popover placement="top" width="250" v-model="data.confirmDelete">
                  <p>Are you sure to delete this?</p>

                  <div style="text-align: right; margin: 10px 0px">
                    <el-button size="mini" type="text" @click="data.confirmDelete = false">Cancel</el-button>
                    <el-button type="primary" size="mini" @click="remove(node, data)">Confirm</el-button>
                  </div>
