        ...
        <el-table-column prop="title_name" sortable label="Titles Name">
          <template slot-scope="scope">
            <div class="text-center">
              <el-button type="text" @click="showUpdatingModal(scope.row.id, scope.row.title)">
                <strong>{{ scope.row.title }}</strong>
              </el-button>
            </div>
          </template>
        </el-table-column>
        <el-table-column v-if="isSocpe('title:destroy')" label="Action" width="300">
          <template slot-scope="scope">
            <img
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        title: [
          { required: true, message: "Please input Title name", trigger: "blur" },
  ...
  computed: {
    ...
    search: function() {
      return this.rows.filter((data) => data.title.toLowerCase().includes(this.searchName.toLowerCase()));
    }
  },
  methods: {
    ...
    isSocpe(scope) {
      return this.tokenInfo["scope"].indexOf(scope) === -1 ? false : true;
    },
    ...
    showUpdatingModal(id, name) {
      // if (this.tokenInfo["scope"].indexOf("title:edit") === -1) return;
      if (!this.isSocpe("title:edit")) return;
      this.currentTitleId = id;
      this.currentTitleName = name;
    ...
    removeTitle: async function() {
      try {
        ...
        this.$toast.success("Remove Title Success");
      } catch (e) {
        this.$toast.error("Remove Title Failed");
      }
      this.getAllTitleData();
      this.isDialogRemove = false;
    },
    ...
    async createTitle() {
      return await TitleService.create(this.form)
        .then((res) => {
          if (res.data) {
            this.$toast.success("Add new title success");
            this.form.title = "";
            this.isDialogCreate = false;
          } else {
    ...
    async updateTitle() {
      if (this.form.title === this.currentTitleName) {
        this.isDialogCreate = false;
        return this.$toast.success("Update title success");
      }
      return await TitleService.update(this.currentTitleId, this.form)
        .then((res) => {
          if (res.data) {
            this.$toast.success("Update title success");
            this.form.title = "";
            this.isDialogCreate = false;
          } else {
            this.$toast.error("Input is not valid");
