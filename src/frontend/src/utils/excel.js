...
export async function generateProbationForm(data){
  ...
  worksheet.getCell('G3').value = data.jobTitle === undefined ? "..........." : data.jobTitle;
  //Team
  worksheet.mergeCells('B4:F4');
  worksheet.getCell('B4').value = {
    'richText': [
      {'font': ExcelStyles.labelFont, 'text': 'Team:'}
    ]
  };
  worksheet.getCell('G4').value = data.probationDetail.team === null ? "..........." : data.probationDetail.team.team_name;
  //Line Manager
  ...
  worksheet.getCell('G5').value = data.probationDetail.probation_line_manager === null ? "..........." : data.probationDetail.probation_line_manager.profile.name;
  //Period of Appraisal
  ...
  worksheet.getCell('G6').value = "From " + data.startDay + " to " + data.endDay;
  ...
  worksheet.getCell('M6').alignment = {vertical: 'middle', horizontal: 'right'};
  ...
  data.competenceData.forEach(competency => {
    ...
    worksheet.getCell(competencyNameCell).value = competency.evaluation_template_competence.competence;
    worksheet.getCell(competencyNameCell).alignment = ExcelStyles.alignment;
    let evaluationStartCol = temp_col;
    data.competenceAssessorRoles.forEach(role => {
      let evaluation = competency.evaluation_template_competence_assessor_role[role]
      if(evaluation) {
  ...
  data.overCmtData.forEach(evaluation => {
    ...
    worksheet.getCell(commentNameCellName).value = evaluation.evaluation_template_overall_comment.term;
    ...
    assessorCell.value = evaluation.assessor === null ? "..............." : evaluation.assessor.profile.name;
    //Assessor Role
    let assessorRoleCellName = getCellName(temp_col + 12, temp_row);
    worksheet.getCell(assessorRoleCellName).value = evaluation.evaluation_template_overall_comment_assessor_role.overall_comment_role;
    temp_row++;
  });
  ...
  worksheet.getCell(startCellName).value = {
    'richText': [
      {'font': ExcelStyles.bodyFont, 'text': data.term}
    ]
  };
  ...
  worksheet.getCell(startCellName).value = {
    'richText': [
      {'font': ExcelStyles.bodyFont, 'text': data.otherUpdateAction}
    ]
  };
  //Director’s approval:
  temp_row ++;
