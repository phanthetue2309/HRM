...
export async function generateProbationList(data) {
  ...
  data.multipleSelection.forEach((listProbation) => {
    ...
    worksheet.getCell(positionCell).value = listProbation.title === null ? "........." : listProbation.employee.title.title;
    //joinDate data
    let joinDateCell = getCellName(temp_col + 3, temp_row);
    worksheet.getCell(joinDateCell).value = listProbation.employee.profile.join_date;
    ...
    worksheet.getCell(typeOfEvaluationCell).alignment = ExcelStyles.centerAlignment;
    temp_row++;
  });
