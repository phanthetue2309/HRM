...
cp -f ${SRC_DIR}/core/sample.config.env ${SRC_DIR}/config.env
...
find ${SRC_DIR}/config.env -type f -name "*" | xargs sed -i'' s/USE_S3=/USE_S3=${USE_S3}/g
...
find ${SRC_DIR}/frontend/.env -type f -name "*" | xargs sed -i'' s%IMAGE_URL=%IMAGE_URL=${IMAGE_URL}%g
find ${SRC_DIR}/frontend/.env -type f -name "*" | xargs sed -i'' s/VALID_TIME=/VALID_TIME=${VALID_TIME}/g
find ${SRC_DIR}/frontend/.env -type f -name "*" | xargs sed -i'' s%BASE_URL=%BASE_URL=${BASE_URL}%g
find ${SRC_DIR}/frontend/.env -type f -name "*" | xargs sed -i'' s%API_URL=%API_URL=${API_URL}%g
find ${SRC_DIR}/frontend/.env -type f -name "*" | xargs sed -i'' s/END_MAIL=/END_MAIL=${END_MAIL}/g
